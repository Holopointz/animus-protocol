...
ASTEROIDS.HUD = class HUD {
    ...
    createDOM() {
        ...
        this.startEl.innerHTML = [
            ...
            '<span style="color:#5ad4ff">Plasma</span> recharges shields · ',
            '<span style="color:#ffb45a">Scrap</span> repairs hull · ',
            '<span style="color:#7dff8a">Electron</span> grants speed. ',
    ...
    update(player, level, gameState) {
        ...
        if (player.foodBuffTimer > 0) {
            var speedMult = player.getFoodSpeedMult ? player.getFoodSpeedMult() : (1 + 0.25 * (player.foodStacks || 1));
            effectsHTML += '<span style="color:#66cc44">SPEED x' + speedMult.toFixed(2) + ' (' + player.foodBuffTimer.toFixed(1) + 's)</span>';
        }
